...
import { getLevel, updateLevel } from "../../utils/api_levels";
...
const EditLevel = () => {
  ...
  // load the level data from the backend API, and assign it the state
  useEffect(() => {
    getLevel(id)
      .then((levelData) => {
        // check if levelData is empty or not
        if (levelData) {
          // update the state with the levelData
          setTitle(levelData ? levelData.title : "");
          setDescription(levelData ? levelData.description : "");
          setBaseDifficulty(levelData ? levelData.baseDifficulty : "");
          setPerfectDifficulty(levelData ? levelData.perfectDifficulty : "");
          setReleaseDate(levelData ? levelData.releaseDate : "");
          setLevelThumbnail(levelData ? levelData.image : "");
        } else {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      // 2. trigger the API to update level
      await updateLevel(id, title, description, baseDifficulty, perfectDifficulty, releaseDate, levelThumbnail, token);
